...
//        initialized
    ...
    @objc func startIcicles() {
        ...
        if isPowerActive == false {
           ...
           stopIcicles()
    Timer.scheduledTimer(timeInterval: TimeInterval(4), target: self, selector: #selector(startIcicles), userInfo: nil, repeats: false)
    ...
    @objc func createIcicle10() {
        let icicle10 = Icicle(name: "icicle10", position: CGPoint(x:  size.width * 0.03 + 500, y: 600), linearDamp:
            1.1)
